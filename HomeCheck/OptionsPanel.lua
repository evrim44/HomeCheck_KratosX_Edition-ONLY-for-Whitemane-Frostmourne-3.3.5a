...
function HomeCheck:OptionsPanel()
    local myOptionsTable = {
        ...
        args = {
            linking = {
                name = "Link to chat/whisper (Left-Click/Shift-Left-Click)",
                desc = "Double-Left-click: Whisper player to use spell.\nShift-Left-click: Post status to Raid/Party chat.\nDisables click-through.",
                type = "toggle",
                set = function(_, val)
            ...
            hideaddon = {
                name = "Hide addon",
                desc = "Completely hide the entire addon, including all title bars and frames.",
                type = "toggle",
                set = function(_, val)
                    self.db.global.hideaddon = val
                    self.addonHidden = val
                    
                    if val then
                        for g = 1, #self.groups do
                            if self.groups[g] then
                                self.groups[g]:Hide()
                            end
                        end
                    else
                        ...
                        for g = 1, #self.groups do
                            if self.groups[g] and self.groups[g].CooldownFrames then
                                -- FIX: Nur einblenden, wenn die Gruppe Balken hat ODER es Gruppe 1 ist
                                if #self.groups[g].CooldownFrames > 0 or g == 1 then
                                    if self.db.profile[g].showTitleBar then
                                        self.groups[g]:Show()
                                    end
                                    for f = 1, #self.groups[g].CooldownFrames do
                                        self.groups[g].CooldownFrames[f]:Show()
                                    end
                                end
                            end
                        end
                ...
                get = function()
                    return self.db.global.hideaddon
                end
            },
            testMode = {
                name = L["Test Mode"],
    ...
    for spellID, spellConfig in pairs(self.spells) do
        if not spellConfig.parent then
            ...
            if self.spells[spellID].improvedloh then
                myOptionsTable.args.spells.args[tostring(spellID)].args.improvedloh = {
                    name = "Improved LoH",
                    type = "toggle",
                    desc = "Show for Paladins with at least 1 point in 'Improved Lay on Hands'",
                    order = 6,
                    set = function(_, val)
                        self.db.profile.spells[spellID].improvedloh = val
                        self:updateRaidCooldowns()
                    end,
                    get = function(_)
                        return self.db.profile.spells[spellID].improvedloh
                    end
                }
            end
    ...
    myOptionsTable.name = "HomeCheck - Kratosx Edition"
